...
import {
  getPendingRequests, acceptFriendRequest,
  declineFriendRequest, getFriends
} from '@/lib/friends'
...
export default function FriendsPage() {
  ...
  // Real-time: new friend request arrives
  useEffect(() => {
    if (!profile) return
    ...
    <div style={{ maxWidth: 620, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div className="feed-tabs" style={{ marginBottom: 24 }}>
        <div className={`feed-tab ${tab === 'requests' ? 'active' : ''}`} onClick={() => setTab('requests')}>
          Friend requests {requests.length > 0 && tab !== 'requests' && (
            <span className="nav-badge" style={{ marginLeft: 6 }}>{requests.length}</span>
          )}
        </div>
      ...
      {loading ? (
        ...
        requests.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {requests.map(req => {
              ...
              const prof = sender?.profession ? PROFESSIONS[sender.profession as keyof typeof PROFESSIONS] : null
              return (
                <div key={req.id} style={{
                  ...
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                    <button
                      ...
                      onClick={() => handleDecline(req.sender_id)}
                    >
                      Decline
                    </button>
                    <button
                      className="btn btn-primary btn-sm"
                      onClick={() => handleAccept(req.sender_id, sender?.full_name)}
                    >
                      Accept
                    </button>
                  </div>
        ...
        friends.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {friends.map(f => {
              const prof = f.profession ? PROFESSIONS[f.profession as keyof typeof PROFESSIONS] : null
              return (
                <div key={f.id} style={{
                  display: 'flex', alignItems: 'center', gap: 14,
                  background: 'var(--surf-1)', border: '1px solid var(--border)',
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <span className="pro-chip">✦ Friends</span>
                  </div>
                </div>
              )
            })}
