...
export default function RightPanel({ onlineFriends, setOnlineFriends, onOpenChat }: Props) {
    ...
    async function load() {
      ...
      if (friendIds.length > 0) {
        ...
        const { data: recentPosts } = await supabase
          .from('posts').select('user_id').in('user_id', friendIds).gte('created_at', cutoff)
      ...
      if (canonical) {
        const [groupsRes, memberRes] = await Promise.all([
          supabase.from('groups').select('id,name,slug,discipline,member_count,post_count')
            .eq('discipline', canonical).order('post_count', { ascending: false }).limit(6),
  ...
  return (
    <div className="p-4 space-y-4">
      {/* Friends + Groups — single apple-card */}
        ...
        <div className="apple-card p-5">
          ...
          {friends.length > 0 && (
            <>
                  ...
                  return (
                    ...
                    >
                      <div className="relative shrink-0">
                        <div className={`w-9 h-9 rounded-full overflow-hidden flex items-center justify-center text-[11px] font-bold ${colorClass} dark:bg-opacity-20`}>
                      ...
                      <p className="text-[14px] font-semibold text-gray-900 dark:text-white truncate group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">
                        {f.full_name}
                      </p>
                    </button>
                  )
              ...
              <button
                className="mt-3 w-full py-1.5 text-[12px] text-brand-600 font-medium hover:bg-brand-50 dark:hover:bg-brand-950/30 rounded-lg transition-colors"
              ...
              <div className="space-y-1">
                {groups.map((g, gi) => {
                  ...
                  return (
                    ...
                    >
                      {/* Stacked avatars */}
                      <div className="flex items-center shrink-0" style={{ width: groupFriends.length > 1 ? 44 : 36 }}>
                        {groupFriends.length > 0 ? groupFriends.map((f, fi) => (
                          <div
                            key={f.id}
                            style={{ marginLeft: fi === 0 ? 0 : -8, zIndex: groupFriends.length - fi }}
                            className={`relative w-7 h-7 rounded-full overflow-hidden flex items-center justify-center text-[8px] font-bold border-2 border-white dark:border-gray-900 ${avatarColors[(gi + fi) % avatarColors.length]} dark:bg-opacity-20`}
                          >
                            {f.avatar_url ? <img src={f.avatar_url} alt="" className="w-full h-full object-cover" /> : initials(f.full_name)}
                          </div>
                        )) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[14px] font-semibold text-gray-900 dark:text-white truncate group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{g.name}</p>
